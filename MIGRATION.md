...
v0.4.0 は以下の **後方互換性のない変更** を含みます。0.x 系のため SemVer 上のバージョンは MINOR 繰り上げ（0.3→0.4）ですが、同一入力で結果が変わりうる破壊的変更のため、本ガイドで移行手順を案内します。なお「意味変更＝MAJOR」の厳格な運用は v1.0.0 到達後から適用します。
...
同一の入力テキストで **v0.3 と v0.4 で結果が変わりうる** 箇所を列挙します。

### ルールデータの変更

| 変更内容 | v0.3 の動作 | v0.4 の動作 |
|---|---|---|
...
v0.4.0 で追加された `journal-rules.json` の新規フィールドです。これらは **オプショナル** であり、未存在のルールは従来通り動作します。
...
## セマンティック バージョニング ポリシー（v1.0.0 以降に厳格適用）
...
| 日時 | 内容 |
|---|---|
| 2026-05-31 | 初版作成（v0.3 → v0.4.0 移行ガイド） |
